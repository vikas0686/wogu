...
`dev.wogu.temporal.callgraph.CallGraphAnalyzer` is this engine, and it's reusable: it takes
...
`dev.wogu.temporal.callgraph.StaticMethodCallTarget` ("this call is a specific method,
...
`dev.wogu.temporal.callgraph.ConstructorCallTarget` ("this is a `new SomeClass(...)` call")
...
and `dev.wogu.temporal.ActivityAwareness` additionally recognizes a method as
